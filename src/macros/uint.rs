    ...
    ($({ $UnsInt:ty, $md:ident, $Ty:ident, $Err:ident, $MinErr:ident, $MaxErr:ident }),+ $(,)*) => {$(
        ...
        pub mod $md {
            ...
            mod tests_uint_specific {
                tests_uint! {
                //  uint, uint, mod_path, TypeName, MinErrorName, MaxErrorName
                    $UnsInt, $md, super, $Ty, $MinErr, $MaxErr
                }
            }
        }
    )+};
...
macro_rules! tests_uint {
    ($UnsInt:ty, $uint_md:ident, $ty_path:path, $Ty:ident, $MinErr:ident, $MaxErr:ident) => {
        ...
        impl_proptest_strategy! {
        //  CnstInt, RhsInt, cnst_int_mod, rhs_int_mod, TypeName
            $UnsInt, $UnsInt, $uint_md, $uint_md, $Ty
        }

        fn assert_add_bounded<const MIN: $UnsInt, const MAX: $UnsInt, const DEF: $UnsInt>(
            rhs: Rhs<{ 0 }, { $Ty::<MIN, MAX, DEF>::range_size() - 1 }>,
            add: impl Fn($Ty<MIN, MAX, DEF>, $UnsInt) -> $Ty<MIN, MAX, DEF>,
        ) {
            let mut cnst = $Ty(MIN);
            cnst = add(cnst, rhs.0);
            assert_eq!(cnst.get(), MIN + rhs.0);
        }
        ...
        >(
            cnst: $Ty<MIN, MAX, DEF>,
            rhs: Rhs<{ $Ty::<MIN, MAX, DEF>::range_size() }, { <$UnsInt>::MAX }>,
            expected: T,
            op: impl Fn($Ty<MIN, MAX, DEF>, $UnsInt) -> T,
        ) {
            let returned = op(cnst, rhs.0);
        ...
        fn assert_wrapping_add_unbounded<
            ...
            const DEF: $UnsInt,
        >(
            rhs: Rhs<{ 1 }, { $Ty::<MIN, MAX, DEF>::range_size() - 1 }>,
            add: impl Fn($Ty<MIN, MAX, DEF>, $UnsInt) -> $Ty<MIN, MAX, DEF>,
        ) {
            let mut cnst = $Ty(MAX);
            cnst = add(cnst, rhs.0);
...
macro_rules! impl_uint_prop_tests_for {
    ({ $UnsInt:ty, $MinErr:ident, $MaxErr:ident }, $({ $md:ident, $Ty:ty }),+ $(,)*) => {$(
        ...
        mod $md {
            ...
            proptest! {
                #[test]
                fn checked_add_bounded(rhs in RhsGen) {
                ...
                #[test]
                fn try_add_bounded(rhs in RhsGen) {
                    assert_add_bounded(rhs, |cnst: Cnst, rhs| {
                ...
                #[test]
                fn saturating_add_bounded(rhs in RhsGen) {
                    assert_add_bounded(rhs, Cnst::saturating_add);
                }

                #[test]
                fn wrapping_add_bounded(rhs in RhsGen) {
                    assert_add_bounded(rhs, Cnst::wrapping_add);
                }

                #[test]
                fn overflowing_add_bounded(rhs in RhsGen) {
                    assert_add_bounded(rhs, |cnst: Cnst, rhs| {
                ...
                #[test]
                fn checked_sub_bounded(rhs in RhsGen) {
                ...
                #[test]
                fn try_sub_bounded(rhs in RhsGen) {
                    assert_sub_bounded(rhs, |cnst: Cnst, rhs| {
                ...
                #[test]
                fn saturating_sub_bounded(rhs in RhsGen) {
                    assert_sub_bounded(rhs, Cnst::saturating_sub);
                }

                #[test]
                fn wrapping_sub_bounded(rhs in RhsGen) {
                    assert_sub_bounded(rhs, Cnst::wrapping_sub);
                }

                #[test]
                fn overflowing_sub_bounded(rhs in RhsGen) {
                    assert_sub_bounded(rhs, |cnst: Cnst, rhs| {
                ...
                #[test]
                fn checked_add_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, None, Cnst::checked_add);
                }

                #[test]
                fn try_add_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, Err(MaxErr::new()), Cnst::try_add);
                }

                #[test]
                fn saturating_add_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, Cnst::new_max(), Cnst::saturating_add);
                }

                #[test]
                fn checked_sub_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, None, Cnst::checked_sub);
                }

                #[test]
                fn try_sub_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, Err(MinErr::new()), Cnst::try_sub);
                }

                #[test]
                fn saturating_sub_unbounded((cnst, rhs) in (CnstGen, RhsGen)) {
                    assert_unbounded(cnst, rhs, Cnst::new_min(), Cnst::saturating_sub);
                }

                #[test]
                fn wrapping_add_unbounded(rhs in RhsGen) {
                    assert_wrapping_add_unbounded(rhs, Cnst::wrapping_add);
                }

                #[test]
                fn overflowing_add_unbounded(rhs in RhsGen) {
                ...
                #[test]
                fn wrapping_sub_unbounded(rhs in RhsGen) {
                    assert_wrapping_sub_unbounded(rhs, Cnst::wrapping_sub);
                }

                #[test]
                fn overflowing_sub_unbounded(rhs in RhsGen) {
